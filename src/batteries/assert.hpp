...
inline std::ostream& fail_check_message(const char* left_str, LeftT&& left_val,
                                        const char* op_str,
                                        const char* right_str,
                                        RightT&& right_val, const char* file,
                                        int line, const char* fn_name)
{
    return std::cerr << "FATAL: " << file << ":" << line
                     << ": Assertion failed: " << left_str << " " << op_str
                     << " " << right_str << "\n (in `" << fn_name << "`)\n\n"
...
#define BATT_CHECK_RELATION(left, op, right)                                   \
    ...
    ::batt::fail_check_message(#left, (left), #op, #right, (right), __FILE__,  \
