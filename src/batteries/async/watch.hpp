#pragma once
#ifndef BATTERIES_ASYNC_WATCH_HPP
#define BATTERIES_ASYNC_WATCH_HPP
   ...
   public:
    ...
    StatusOr<T> await_not_equal(const T& last_seen) const
    {
        BATT_DEBUG_INFO("WatchAtomic{value="
                        ...
                        << ") observers.size()=" << this->observers_.size()
                        << " is_closed=" << this->is_closed() << " last_seen=" << last_seen << "}");
...
#endif  // BATTERIES_ASYNC_WATCH_HPP
