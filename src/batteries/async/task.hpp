    ...
    : public boost::intrusive::list_base_hook<boost::intrusive::link_mode<boost::intrusive::auto_unlink>>
{
   ...
   public:
    ...
    // resumed.
    //
    static constexpr u32 kSleepTimerLockSuspend = u32{1} << 7;
    ...
    //
    static constexpr bool is_running_state(u32 state)
    {
    ...
    //
    static constexpr bool is_ready_state(u32 state)
    {
        return
    ...
    //
    static constexpr bool is_terminal_state(u32 state)
    {
    ...
    template <std::size_t kSize>
    static auto& stack_allocator()
    {
        // using TaskStack = boost::context::fixedsize_stack;
        using TaskStack = boost::context::protected_fixedsize_stack;
        static TaskStack a{kSize};
        return a;
    }
    ...
    static R await(Fn&& fn)
    {
        ...
        //
        HandlerMemory<512> handler_memory;
        std::promise<R> prom;
    ...
    template <typename BodyFn, std::size_t kStackSize = 512 * 1024>
    explicit Task(const boost::asio::any_io_executor& ex, BodyFn&& body_fn,
                  std::string&& name = "(anonymous)", StackSize<kStackSize> = {}) noexcept
        : name_(std::move(name))
        , ex_(ex)
    {
        this->self_ = callcc(  //
   ...
   private:
    ...
    {
       public:
        explicit SpinLockGuard(Task* task, u32 mask) noexcept : task_{task}, mask_{mask}
        {
    ...
    //
    void schedule_to_run(u32 observed_state, bool force_post = false);
    ...
    R await_impl(Fn&& fn)
    {
        ...
        const u32 prior_state = this->state_.fetch_or(kNeedSignal);
        BATT_CHECK_NE((prior_state & kHaveSignal), kHaveSignal)
            << "prior_state=" << std::bitset<8>{prior_state};
    ...
    // - kTerminated
    //
    void handle_event(u32 event_mask);
    ...
    //
    bool try_spin_lock(u32 lock_mask, u32& prior_state);
    ...
    //
    void spin_unlock(u32 lock_mask);
    ...
    Continuation self_;
    std::atomic<u32> state_{kSuspended};
    ...
    HandlerMemory<128> activate_memory_;
