...
BATT_INLINE_IMPL Grant& Grant::subsume(Grant&& that)
{
    if (!this->issuer_) {
        *this = std::move(that);
        ...
        if (BATT_HINT_FALSE((this->issuer_ != that.issuer_))) {
            BATT_CHECK_EQ(that.size_.get_value(), 0u)
                << BATT_INSPECT((void*)this->issuer_.get()) << BATT_INSPECT((void*)that.issuer_.get())
