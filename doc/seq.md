...
| <nobr>[product()][function-sum]</nobr> | Returns the arithmetic product of the items in the input sequence.  The item type `T` of the input sequence must support built-in multiplication (`#!cpp operator*`), and `T{1}` must be the multiplicative identity value of `T` (i.e., "one").<br><br>`#!cpp #include <batteries/seq/product.hpp>` |
